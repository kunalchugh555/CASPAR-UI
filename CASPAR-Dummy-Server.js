...
wss.on('connection', function connection(ws) {
  ...
  let lineReader = readline.createInterface({
      input: fs.createReadStream('./configurations/configs.txt')
    })
    lineReader.on('line', function (line) {
      if (line.includes("cname: "))
          {
            configstrings = configstrings + ":;:;:" + (line.substring(line.lastIndexOf("cname: ") + 7)).trim();
            confignamesend = configstrings;
...
//Kunal
          //case "configrequest":
            //console.log("works");
              //let stringsend = " ";
             // let configname = msg.config;
            //  var lineReader = readline.createInterface({
            //      input: fs.createReadStream('./configurations/configs.txt')
            //  });
            //  count = -1;
            //  lineReader.on('line', function (line) {
            //      if (line.includes("cname: " + configname)){
             //         count = 3;
             //     }
              //    if (count>0){
              //        stringsend = stringsend + ":;:;:" + line;
             //     }
             //         count = count-1;
             // });
              //sendConfigData(stringsend);
